...
function main() {
  ...
  console.log("RelayDB Columnar Artifact v1 Stress Test");
  console.log("========================================");
  ...
  return [
...

    {
      ...
      run: () => {
        ...
        {
      name: "Q8b group by status x agriculture bitsets",
      run: () => {
        const active = db.bitsets.activePeople;
        const inactive = getStatusBitsetCached(db, STATUS_INACTIVE);
        const pending = getStatusBitsetCached(db, STATUS_PENDING);
        const agriculture = db.bitsets.agriculturePeople;

        const activeTotal = countSetBits(active);
        const inactiveTotal = countSetBits(inactive);
        const pendingTotal = countSetBits(pending);

        const activeAgriculture = countAndSetBits(active, agriculture);
        const inactiveAgriculture = countAndSetBits(inactive, agriculture);
        const pendingAgriculture = countAndSetBits(pending, agriculture);

        const counts = {
          activeAgriculture,
          activeOther: activeTotal - activeAgriculture,

          inactiveAgriculture,
          inactiveOther: inactiveTotal - inactiveAgriculture,

          pendingAgriculture,
          pendingOther: pendingTotal - pendingAgriculture,
...
function countAndSetBits(bitsetA, bitsetB) {
  let count = 0;

  for (let i = 0; i < bitsetA.length; i += 1) {
    count += popcount32(bitsetA[i] & bitsetB[i]);
  }
